...
hfp_method() {
    ...
    genrules "consumption_${month}_train.csv-sr.fis"
    fpa "config.fis" "consumption_${month}_train.csv" -f"output.fis" -t"consumption_${month}_test.csv"
...
if [ ! -z "${output}" ]; then
	...
	then
		...
		for m in ${months[@]}; do
			cd $m
			rm -f *".fis" *".mat" *".ols" *".perf" *".res" *".fpa" "info.gen" *".vertex" *".hfp"
			cd ..
		done
	fi
else
